...
import { DotsThreeVerticalIcon, ClockIcon } from "@phosphor-icons/react";
...
const Page = () => {
  ...
  const pollExecution = React.useCallback(async (executionId: string) => {
    try {
      ...
      if (execution.status === "success" || execution.status === "failed") {
        ...
        if (currentThreadId && (execution.status === "success" || execution.status === "failed")) {
          setTimeout(async () => {
            try {
                    ...
                    if (thread.thread) {
                      ...
                      if (!threadTitle && allMessages.length >= 2) {
                        ...
                        if (firstUserMsg) {
                          ...
                          const words = firstUserMsg.content.split(" ").slice(0, 5);
          ...
          setMessages((prev) => {
            ...
            if (
              ...
              if (isReasoning) {
                ...
                const currentStreaming = newMessages[lastAssistantIndex].streamingContent || "";
                newMessages[lastAssistantIndex].streamingContent = currentStreaming + data.delta;
              }
            }
            return newMessages;
          });
    ...
    if (!threadId) {
      try {
        const threadResult = await api.createThread({
          ...
          metadata: {
            messages: [],
            githubRepo: githubRepo ? { owner: githubRepo.owner, repo: githubRepo.repo } : undefined,
          },
        });
  ...
  return (
    <div className="flex max-w-7xl mx-auto h-full w-full">
      <div className="bg-black/20 flex flex-col justify-between overflow-hidden border-2 border-black/40 w-3/3 h-156 rounded-4xl">
        <div className="bg-white/1 flex justify-between items-center w-full p-5">
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-bold">
              {agent?.name ||
                (agent === null ? "Agent Name" : "Loading...")}
            </h1>
            <button
              ...
              title="View history"
            >
              <ClockIcon className="text-white/70 text-lg" />
            </button>
            {loading && currentExecutionIdRef.current && (
              <div className="flex items-center gap-2">
                <div className="flex gap-1">
                  <span className="inline-block w-2 h-2 bg-[#36B460] rounded-full animate-pulse" style={{ animationDelay: '0ms' }} />
                  <span className="inline-block w-2 h-2 bg-[#36B460] rounded-full animate-pulse" style={{ animationDelay: '150ms' }} />
                  <span className="inline-block w-2 h-2 bg-[#36B460] rounded-full animate-pulse" style={{ animationDelay: '300ms' }} />
                </div>
                <button
                  onClick={async () => {
                    if (currentExecutionIdRef.current) {
                      try {
                        await api.cancelExecution(currentExecutionIdRef.current);
                        setLoading(false);
                        currentExecutionIdRef.current = null;
                        if (pollingIntervalRef.current) {
                          clearInterval(pollingIntervalRef.current);
          ...
          <button
            ...
            className="bg-black/20 hover:bg-black/30 rounded-full p-3 transition-colors"
            title="Edit agent"
          >
            <DotsThreeVerticalIcon className="text-lg text-white" />
          </button>
        </div>
        <div className="flex gap-5 h-full overflow-y-scroll p-6 flex-col">
          {messages.length === 0 && (
      ...
      <HistorySidebar
        ...
        onSelectThread={async (threadId) => {
          // Load thread and show messages in chat
          try {
            ...
            if (thread.thread) {
              ...
              const chatMessages: ChatMessage[] = threadMessages.map((msg: any, idx: number) => ({
                id: `${threadId}-${idx}`,
                role: msg.role as "user" | "assistant",
                content: msg.content || "",
                timestamp: new Date(thread.thread.updatedAt || thread.thread.createdAt),
              }));
