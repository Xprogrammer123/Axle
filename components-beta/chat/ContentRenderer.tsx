...
export const ContentRenderer: React.FC<ContentRendererProps> = ({
  ...
  if (type === "code") {
    ...
    while ((match = codeBlockRegex.exec(content)) !== null) {
      ...
      if (match.index > lastIndex) {
        parts.push(
          <p key={`text-${key++}`} className="font-medium dark">
      ...
      parts.push(
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs dark/50 font-mono">{language}</span>
            <button
              onClick={() => navigator.clipboard.writeText(code)}
              className="text-xs dark/50 hover:dark transition-colors"
            >
              Copy
            </button>
          </div>
          <pre className="text-sm dark/90 font-mono whitespace-pre-wrap">
            <code>{code}</code>
          </pre>
        </div>
      );
        ...
        {sections.map((section, index) => {
          ...
          return (
            <div key={index} className={isHeader ? "mb-4" : ""}>
              {isHeader ? (
                <h3 className="text-lg font-bold dark mb-2">
                  {section.trim()}
                </h3>
              ) : (
                <p className="dark/90 leading-relaxed">
                  {section.trim()}
                </p>
              )}
            </div>
        ...
        {lines.map((line, index) => {
          ...
          if (line.startsWith("#")) {
            return (
              <h2
                key={index}
                className="text-xl font-bold dark mt-4 mb-2"
              >
                {line.replace(/^#+\s*/, "")}
              </h2>
            );
          ...
          if (line.startsWith("-") || line.startsWith("*")) {
            return (
              <li key={index} className="dark/90 ml-4 list-disc">
                {line.substring(1).trim()}
              </li>
            );
          }
          return (
            <p key={index} className="dark/90 leading-relaxed">
              {line}
            </p>
          );
        })}
  ...
  return (
    <div className="dark/90 leading-relaxed whitespace-pre-wrap">
      {content}
      {isStreaming && (
