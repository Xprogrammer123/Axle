...
export const ToolCallCard: React.FC<ToolCallCardProps> = ({
  ...
  const cardContent = (
    <div
      className={`flex w-fit bg-dark/2 border-4 border-dark/3 rounded-3xl p-3.5 pr-5 items-center gap-3 transition-all duration-150 ${
        isRunning
          ? "animate-pulse border-[#36B460]/40 shadow-lg shadow-[#36B460]/30 scale-[1.02]"
          : actionLink
          ? "hover:scale-105 cursor-pointer hover:bg-dark/3 hover:border-dark/10 hover:shadow-lg transition-transform"
    ...
    >
      ...
      <div className="flex flex-col gap-0.5 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          <p
            className="text-dark font-semibold text-sm truncate max-w-[180px]"
            title={actionDescription}
          >
            {actionDescription}
          </p>
          {status === "running" && (
            <span className="text-xs text-dark/50 darkspace-nowrap">
              Running...
            </span>
          )}
          {status === "success" && !isRunning && (
            <span className="text-xs text-[#36B460] darkspace-nowrap">✓</span>
          )}
          {status === "failed" && (
            <span className="text-xs text-red-500 darkspace-nowrap">✗</span>
          )}
        </div>
