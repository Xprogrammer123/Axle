  ...
  return (
    <div className="flex flex-col gap-2">
      <button
        ...
        className="flex text-white/35 text-[13px] items-center gap-1 cursor-pointer hover:text-white/50 transition-colors"
      >
        {isStreaming ? (
          <>
        ...
        <CaretDownIcon 
          className={`text-[15px] text-white/35 transition-transform ${isOpen ? 'rotate-180' : ''}`} 
        />
      </button>
      {isOpen && (
        <div className="bg-black/20 border border-white/5 rounded-xl p-3 mt-1 max-h-64 overflow-y-auto">
          <p className="text-white/70 text-sm whitespace-pre-wrap">{thought}</p>
          {isStreaming && (
